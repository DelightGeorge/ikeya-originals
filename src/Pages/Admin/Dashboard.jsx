  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ FIXED: Fetch all products instead of just recent
                ...
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h3 className="text-xs uppercase tracking-widest font-bold flex items-center gap-2">
                      <LayoutDashboard size={14} /> All Products
                    </h3>
                    <p className="text-[9px] text-neutral-400 mt-1">
                      Showing {startIdx + 1}-{Math.min(endIdx, allProducts.length)} of {allProducts.length} products
                    </p>
                  </div>
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between mt-8 pt-8 border-t border-neutral-100">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-neutral-400 hover:text-black disabled:opacity-50 disabled:cursor-not-allowed px-4 py-2 border border-neutral-200 hover:border-black transition-all"
                        >
                        ...
                        <div className="flex items-center gap-2">
                          {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                            <button
                              ...
                              className={`w-8 h-8 text-[9px] font-bold uppercase transition-all ${
                                currentPage === page
